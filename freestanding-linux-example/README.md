...
Using this allocator approach is in contrast to libc that uses `brk` and `mmap`
syscalls for that. However, this is just an example. Under Linux, you always
want to use `mmap` etc. However, if you have a freestanding binary without
available memory management, simple chunk allocator is a nice and convenient
way to go with a nice API.